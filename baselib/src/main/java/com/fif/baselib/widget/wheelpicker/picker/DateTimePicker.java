...
public class DateTimePicker extends WheelPicker {
    ...
    public void setDateRangeStart(int startYearOrMonth, int startMonthOrDay) {
        ...
        if (dateMode == YEAR_MONTH) {
            ...
            @SuppressLint("WrongConstant") int year = Calendar.getInstance(Locale.CHINA).get(Calendar.YEAR);
    ...
    private void initYearData() {
        ...
        if (!resetWhileWheel) {
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                @SuppressLint("WrongConstant") int index = years.indexOf(DateUtils.fillZero(Calendar.getInstance().get(Calendar.YEAR)));
                if (index == -1) {

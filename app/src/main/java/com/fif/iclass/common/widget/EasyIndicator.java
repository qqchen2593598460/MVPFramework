...
public class EasyIndicator extends LinearLayout implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private void setSelectorColor(TextView tv) {
        for (TextView tv1 : tvs) {
            tv1.setTextColor(indicator_normal_color);
            tv1.setTextSize(TypedValue.COMPLEX_UNIT_PX, indicator_textSize);
        }
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        //防止超出左边屏幕
        if (positionOffsetPixels < 1) return;
